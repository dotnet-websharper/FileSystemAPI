...
function Main(){
  ...
  const b=(t.h.push(EventQ2(t.k, "opendirectory", () => t.i, () => {
    const _3=null;
    StartImmediate(Delay(() => Bind(AsAsync(openDirectory()), () => Return(null))), null);
  })),t);
...
function openDirectory(){
  ...
  return new Promise((_2) => {
    _2(new Promise((_3) => {
      _3(globalThis.showDirectoryPicker().then((d) => {
        const fileList=globalThis.document.getElementById("fileList");
        fileList.innerHTML="";
        const iterator=d.values();
        function loop(){
          const _4=null;
          return new Promise((_5) => {
            _5(OfAsync(AsAsync(iterator.next())).then((d_1) => {
              if(!(d_1==null)){
                if(!d_1.done){
                  const entry=d_1.value;
                  const name=entry.name;
                  const icon=entry.kind=="directory"?" \ud83d\udcc1":" \ud83d\udcc4";
                  const listItem=globalThis.document.createElement("li");
                  listItem.textContent=String(name)+String(icon);
        ...
        return loop().then(() => Promise.resolve(null));
      }));
    })["catch"]((d) => {
      console.error("Error accessing directory: "+String(unwrapExn(d).message));
